...
local function hopToLowPlayerServer()
            ...
            for _, server in pairs(result.data) do
                if type(server) == "table" and server.playing and server.id and server.playing >= 1 and server.playing <= 2 and server.id ~= game.JobId then
                    ...
                    if #suitableServers >= 10 then
                        ...
                        wait(1)
                        local ok = teleportToServer(suitableServers[10].id)
                        if not ok then
                            updateStatus("Failed!", Color3.fromRGB(255, 0, 0))
                            wait(2)
                            updateStatus("Ready", Color3.fromRGB(0, 255, 127))
    ...
    if #suitableServers > 0 then
        ...
        wait(1)
        local ok = teleportToServer(suitableServers[#suitableServers].id)
        if not ok then
            updateStatus("Failed!", Color3.fromRGB(255, 0, 0))
            wait(2)
            updateStatus("Ready", Color3.fromRGB(0, 255, 127))
...
-- Bắt lỗi teleport thất bại
TeleportService.TeleportInitFailed:Connect(function(plr, teleportResult, errorMessage)
    warn("TeleportInitFailed:", tostring(teleportResult), errorMessage)
    updateStatus("Err!", Color3.fromRGB(255, 0, 0))
    wait(2)
    updateStatus("Ready", Color3.fromRGB(0, 255, 127))
    isHopping = false
end)
